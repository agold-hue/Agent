...
function baseUrl(): string {
  return (process.env.LLM_BASE_URL || "https://openrouter.ai/api/v1").replace(/\/$/, "");
}
...
export async function complete(opts: {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const res = await fetch(`${baseUrl()}/chat/completions`, { method: "POST", headers, body: JSON.stringify(body), signal: opts.signal });

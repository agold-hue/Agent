...
export async function loginToSite(t: Tenant, opts: {
  ...
  try {
    ...
    let reached = await reachLoginForm(page, domain, profile);
    if (!reached.found && BOT_WALL.test(await pageText(page).catch(() => ""))) {
      ...
      if (await waitOutBotWall(page, 20_000)) reached = await reachLoginForm(page, domain, profile);
    }
    if (!reached.found) {

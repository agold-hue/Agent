...
import { activityOf, chatHistory, currentChatSession, recentNotices } from "../../../lib/chat.js";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const [items, notices, liveView] = await Promise.all([chatHistory(t, session), recentNotices(t, 30).catch(() => []), wantsLive ? liveViewIfRunning(session!.browserbase_session_id!) : Promise.resolve(null)]);
  ...
  return res.status(200).json({ session_id: session?.id ?? null, status: session?.status ?? "none", pending: session?.pending_kind ?? null, activity: activityOf(session), live_view_url: liveView, items: merged });
}

 ...
 * secrets). POST { confirm: <email> } -> deletes the account and everything under it.
 */
export default async function handler(req: VercelRequest, res: VercelResponse) {
  const t = await requireTenant(req, res, { paid: false });
  if (!t) return;
  if (req.method === "GET") {
    const [memories, sessions, items, receipts, wins, followups, orders, logins] = await Promise.all([
    ...
    return res.status(200).send(JSON.stringify({ exported_at: new Date().toISOString(), account: { email: t.email, name: t.name, timezone: t.timezone, settings: t.settings }, memories, sessions, items, receipts, wins, followups, standing_orders: orders, logins }, null, 2));
  }
  if (req.method === "POST") {
